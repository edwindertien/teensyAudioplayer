...
│                                                         │
│  Pin  3 ──── 1kΩ ──┬──── transducer amp IN  Haptic PWM  │
│  Pin  4 ──── 270kΩ─┘                                    │
│               └──── 100nF to GND  (RC low-pass ~1.6kHz) │
└─────────────────────────────────────────────────────────┘
```

> **Note:** The Teensy 4.1 has no DAC. PWM output on pins 3 (MSB) + 4 (LSB)
...
The experience designer edits only this file — no reflashing needed. The file can be visualised using a python script shown below. 
...
![experience map](docs/interactiondiagram.png)
